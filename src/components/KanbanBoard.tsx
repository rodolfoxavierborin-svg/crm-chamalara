...
export default function KanbanBoard({ onSelectLead }: { onSelectLead?: (lead: any) => void }) {
  ...
  const leadsNaUnidade = leadsNoPeriodo.filter((lead) => unidadeFilter === 'all' ? true : (lead.unidade || 'Pendente') === unidadeFilter);
  ...
  const gerarDadosEvolucaoDeterministica = () => {
    ...
    if (dateRange === 'today') {
      // 7 intervalos de horário no dia de hoje (08:00 às 20:00)
      ...
      // 7 dias retroativos a partir de hoje (ou do filtro de data)
  ...
  // Construção do Path SVG com base nos valores numéricos reais
  const svgWidth = 600;
  const svgHeight = 200;
  const paddingX = 40;
  const paddingY = 30;
  ...
  const maxVal = Math.max(...chartBuckets.map((b) => b.total), 5); // Valor máximo para dimensionamento proporcional
    ...
    <div className="flex flex-col h-full bg-[#F4F6F8] text-slate-800 font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-5 flex flex-wrap items-center justify-between z-10 sticky top-0 shadow-sm gap-6">
        ...
        <div className="flex items-center gap-4">
          <select value={unidadeFilter} onChange={(e) => setUnidadeFilter(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm text-slate-700 outline-none cursor-pointer focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
